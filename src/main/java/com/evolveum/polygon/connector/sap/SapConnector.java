        ...
        if (activityGroups != null && activityGroups.size() > 0) {
            for (Item newItem : activityGroups.getValues()) {
                Optional<Item> matchingOldValue = tableOldActivitygroups.getValues()
                                                                        .stream()
                                                                        .filter(item ->
                                                                                    // if indirect groups will be hidden, dont match them as an old candidate for merging
                                                                                    (!configuration.getHideIndirectActivitygroups()  || StringUtil.isBlank(item.getByAttribute("ORG_FLAG")))
                                                                                    && Objects.equals(
